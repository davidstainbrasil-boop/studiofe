  ...
  async generateSubtitles(
    ...
    try {
      ...
      if (videoFile.size > maxSize) {
        throw new Error('Video file too large. Please use a file smaller than 25MB or extract audio first.')
      }

      console.log('Calling OpenAI Whisper API...')
      
      // Call Whisper API with proper typing
      ...
      const subtitles = segments.map((segment: any, index: number) => ({
        id: index + 1,
      ...
      // Calculate metadata
      const detectedLanguage = (transcription as any).language || language
      const duration = segments.length > 0 
        ? segments[segments.length - 1].end 
        : 0
      const wordCount = this.countWords((transcription as any).text || '')
