...
describe('WebSocket Server - Testes Unitários', () => {
  ...
  const waitForActiveUsers = (
    ...
    return new Promise((resolve, reject) => {
      ...
      const cleanup = () => {
        ...
        socket.off(TimelineEvent.ACTIVE_USERS, onActiveUsers)
      }
      ...
      const timeout = setTimeout(() => {
        ...
        settled = true
        cleanup()
        reject(new Error(`Timeout aguardando ACTIVE_USERS (projectId=${projectId}, expectedCount=${expectedCount})`))
      }, timeoutMs)

      const onActiveUsers = (data: { projectId: string; users?: string[]; count?: number }) => {
        if (data.projectId !== projectId) return

        const count = typeof data.count === 'number' ? data.count : (data.users?.length ?? 0)
      ...
      socket.on(TimelineEvent.ACTIVE_USERS, onActiveUsers)

      const poll = () => {
        socket.emit('timeline:get_active_users', { projectId })
      }
  ...
  describe('Lock/Unlock de Tracks', () => {
    it('deve bloquear track e notificar outros usuários', (done) => {
      ...
      const checkBothConnected = () => {
        ...
        if (bothConnected === 2) {
          Promise.all([
            waitForActiveUsers(clientSocket1, 'proj_test_1', 2),
            waitForActiveUsers(clientSocket2, 'proj_test_1', 2)
          ])
            .then(() => {
              // Ambos já entraram na room, User 1 bloqueia track
      ...
      const checkBothConnected = () => {
        ...
        if (bothConnected === 2) {
          Promise.all([
            ...
            .then(() => {
              // User 1 desbloqueia track (garantindo que ambos já entraram na room)
  ...
  describe('Presence e Cursores', () => {
      ...
      const checkBothConnected = () => {
        ...
        if (bothConnected === 2) {
          Promise.all([
            ...
            .then(() => {
              // User 1 move cursor (com ambos na room)
    ...
    it('deve listar usuários ativos no projeto', (done) => {
      ...
      waitForActiveUsers(clientSocket3, 'proj_test_1', 3)
        .then(() => {
          if (socket3Joined) return
          socket3Joined = true
          clientSocket3.emit('timeline:get_active_users', { projectId: 'proj_test_1' })
        })
        .catch(fail)

      clientSocket3.on(TimelineEvent.ACTIVE_USERS, (data) => {
        if (!socket3Joined) return
        if (data.projectId !== 'proj_test_1') return
        if (data.users.length !== 3) return
