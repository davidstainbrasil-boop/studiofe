...
jest.mock('child_process', () => ({
  exec: jest.fn((cmd, cb) => cb(null, { stdout: 'ok', stderr: '' }))
}));

jest.mock('fs/promises', () => ({
  writeFile: jest.fn(),
  readFile: jest.fn().mockResolvedValue(Buffer.from('fake-audio-content')),
  mkdir: jest.fn(),
}));
  ...
  getServiceRoleClient: () => ({
    storage: {
      from: () => ({
        ...
        getPublicUrl: (path: string) => ({ data: { publicUrl: `https://fake-supabase.com/${path}` } })
      })
    }
  })
}));
...
describe('TTS Service Tests', () => {
  describe('Basic Functionality', () => {
    ...
    test('should use fallback when API key missing', async () => {
      ...
      expect(result).toHaveProperty('fileUrl');
      // expect(result.fileUrl).toMatch(/\/tts\/.*\.mp3$/); // Ajustado para corresponder ao mock do supabase
      expect(result.fileUrl).toContain('https://fake-supabase.com/tts/');
    });
  });
