...
import { createFileObject, cleanupTestFiles } from '../../helpers/test-helpers';
...
describe('PPTX Processing Real - Fase 1', () => {
  ...
  describe('PPTXTextParser', () => {
    it('deve extrair texto do slide 1', async () => {
      const textResult = await textParser.extractTextFromSlide(testZip, 1)

      expect(textResult.text).toBeDefined() // Propriedade correta é 'text' não 'plainText'
      expect(textResult.text).toContain('Slide 1 Title')
      expect(textResult.wordCount).toBeGreaterThan(0)
    })

    it('deve extrair texto do slide 2 com bullets', async () => {
      const textResult = await textParser.extractTextFromSlide(testZip, 2)
      
      expect(textResult.text).toBeDefined()
      // O parser atual retorna text, lines, wordCount. Não vi bulletPoints na interface SlideTextExtractionResult lida anteriormente, mas vi na interface.
      // Vamos verificar a interface novamente:
      // export interface SlideTextExtractionResult { ... bulletPoints?: string[]; ... }
      expect(Array.isArray(textResult.bulletPoints)).toBe(true)
    })

    it('deve extrair texto de todos os slides', async () => {
      for (let i = 1; i <= 5; i++) {
        const textResult = await textParser.extractTextFromSlide(testZip, i)
    ...
    it('deve retornar vazio para slide inexistente', async () => {
        // A implementação atual retorna objeto vazio com text: ''
      const textResult = await textParser.extractTextFromSlide(testZip, 99)
  ...
  describe('PPTXImageParser', () => {
    it('deve processar extração de imagens sem erro', async () => {
      const imageResult = await imageParser.extractImages(
        testZip,
        1, // slideNumber
        {
          includeDataUrl: true
      ...
      expect(Array.isArray(imageResult)).toBe(true)
      expect(imageResult.length).toBe(0) // PPTX de teste não tem imagens
    })

    // Removendo teste de generateThumbnail pois o método não existe na classe atual
  })
