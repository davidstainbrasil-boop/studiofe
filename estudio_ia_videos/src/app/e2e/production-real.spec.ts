...
test.describe('Production Critical Flow', () => {
  ...
  test('01. Home Page Load & Static Assets', async ({ page }) => {
    ...
    const cssLoaded = await page.evaluate(() => {
        return Array.from(document.styleSheets).some(sheet => sheet.href && sheet.href.includes('_next/static/css'));
    });
  ...
  test('03. Dashboard Data Integrity', async ({ page }) => {
     ...
     const dashboardHeader = page.locator('h1, h2').filter({ hasText: /Dashboard|Estúdio/ }).first();
     await expect(dashboardHeader).toBeVisible();

     // Check for lack of error banner
     const errorBanner = page.locator('div, span, p').filter({ hasText: /Erro ao carregar|Failed to fetch/ });
