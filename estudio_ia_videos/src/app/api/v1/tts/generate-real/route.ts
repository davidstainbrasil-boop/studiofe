    ...
    if (action === 'voices') {
      ...
      return NextResponse.json({
        ...
        project: {
          ...
          totalDuration: (project.slides as Array<{ duration?: number; durationSeconds?: number }>).reduce((sum: number, slide: { duration?: number; durationSeconds?: number }) => sum + (slide.duration || slide.durationSeconds || 5), 0)
        },
        slides: {
