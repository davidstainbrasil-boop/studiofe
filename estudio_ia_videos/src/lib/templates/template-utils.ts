/**
 * Template Utils
 * Client-safe utilities for applying templates
 */
...
/**
 * Apply template to project (Client-Side Helper)
...
export function applyTemplateToProject(
  ...
  if (!currentProject) {
    const initialElements = cloneTemplateElements(template.elements, 0);
    return {
  ...
  targetLayer.elements.push(...newElements);
